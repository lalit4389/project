...
import { TrendingUp, TrendingDown, DollarSign, Activity, Copy, CheckCircle, Bot } from 'lucide-react';
...
const Overview: React.FC = () => {
  ...
  const webhookUrl = `${window.location.origin}/api/webhook/123`; // Replace with actual user ID
  ...
  const fetchDashboardData = async () => {
    try {
      const [pnlResponse, positionsResponse, ordersResponse] = await Promise.all([
  ...
  return (
    <div className="space-y-8">
      ...
      >
        ...
        <div className="flex items-center space-x-3 bg-dark-900/50 rounded-xl p-4 border border-olive-500/10">
          <code className="flex-1 text-sm text-olive-300 break-all font-mono">
            {webhookUrl}
          </code>
          <motion.button
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <button className="text-olive-400 hover:text-olive-300 font-medium transition-colors">
            View All
          </button>
        </div>
                ...
                >
                  ...
                  <td className="py-3 px-4">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      order.side === 'BUY' ? 'bg-olive-800/30 text-olive-300' : 'bg-red-800/30 text-red-300'
                    }`}>
                      {order.side}
                    </span>
                  </td>
                  ...
                  <td className="py-3 px-4">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      order.status === 'EXECUTED' ? 'bg-olive-800/30 text-olive-300' :
                      order.status === 'PENDING' ? 'bg-yellow-800/30 text-yellow-300' :
